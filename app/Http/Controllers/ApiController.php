        ...
        if ($request->product_id !== null) {
            ...
            foreach ($products as $index => $item) {
                foreach ($item as $subIndex => $subItem) {
                    $order_detail = new order_details();
                    $order_detail->product_id = (int)$subItem['product_id'];
                    $order_detail->user_id = $request->user_id;
                    $order_detail->order_id = $order->id;
                    $order_detail->option1 = (int)$subItem['option1Id'];
                    $order_detail->option2 = (int)$subItem['option2Id'];
                    $order_detail->num = (int)$subItem['num'];
                    $order_detail->save();
                }
            }
            return response()->json([
